...
// 1 类组件： observer用于触发视图更新, inject是为了获取到APP中基于Provider放在上下文中的各个模块的Store实例，以属性props的形式传递给组件
// @observer
...
const Task = function Task(props) {
    let {task} = props; //基于props获取task模块的TaskStore实例
    /* 表格列的数据 */
    const columns = [{
        title: '编号',
        dataIndex: 'id',
    ...
    /* 关于TABLE和数据的处理 */
    useEffect(()=>{
        const tmpAsyncFunc = async() => {
            if(!task.list){
                setTableLoading(true);
                await task.queryAllTaskAction();
                setTableLoading(fasle);
            }
        }
    },[]);
    ...
    useEffect(()=>{
        ...
        if(selectedIndex !== 0){
            taskList = taskList.filter(_ => {
                return _.state === selectedIndex;
            });
        }
...
export default inject('task','personal')(observer(Task));// 先把observer(Task)装饰器执行，没有return就会返回自己，所以这个执行结果作为参数继续传递给inject('task','personal')执行的结果(是个装饰器)执行
